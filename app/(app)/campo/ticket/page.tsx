...
import { TicketCampoForm } from "./ticket-campo-form";

export const metadata = { title: "Reportar incidente" };
export const dynamic = "force-dynamic";

export default async function CampoTicketPage({
  searchParams,
}: {
    ...
    return (
      <div className="mx-auto w-full max-w-md px-4 py-6">
        ...
        <p className="mb-5 text-[13px] text-ink-3">
          Elige el proyecto donde ocurre el problema.
        </p>
        {(proyectos ?? []).length === 0 ? (
          ...
          <ul className="space-y-2">
            {(proyectos ?? []).map(
              (p: { id: string; codigo: string; nombre: string }) => (
                <li key={p.id}>
                  <Link
                    href={`/campo/ticket?proyecto=${p.id}`}
    ...
    return (
      <div className="mx-auto w-full max-w-md px-4 py-6">
        <p className="rounded-md border border-dashed border-border bg-card p-6 text-center text-[13px] text-ink-3">
          ...
          <Link href="/campo/ticket" className="text-brand hover:underline">
            Elegir otro
          </Link>
        </p>
      </div>
