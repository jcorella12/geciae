...
export async function HeroMargenEmpresa({
  empresaId,
}: {
  ...
  if (!empresaId) {
    return (
      <div className="text-[12px] text-ink-3">
        Selecciona una empresa específica para ver su margen.
      </div>
    );
  }
  ...
  const { data } = (await (supabase as any)
    ...
    .select("ingreso_presupuestado, costos_totales, margen_neto")
    .eq("empresa_id", empresaId)) as unknown as {
    data:
      | {
